...
KSP (Kerbal Space Program 1.12.5) mod providing an in-game AI chat assistant using Unity uGUI, connected to any OpenAI- or Anthropic-compatible LLM API. The provider protocol is auto-detected from the configured Base URL (URLs containing `api.anthropic.com` or ending in `/anthropic` use the Anthropic Messages API; everything else uses OpenAI Chat Completions).
...
src/
  ...
  Api/                       # LLM communication
    ChatProvider.cs          # ChatProvider enum + ChatProviderDetector (auto-detect OpenAI vs Anthropic from Base URL)
    LlmClient.cs             # API client using UnityWebRequest with SSE streaming + tool call parsing; branches on provider for URL/headers/body, shared SseDownloadHandler takes a parser delegate
    JsonHelper.cs            # Manual JSON utilities: escape, extract, build OpenAI/Anthropic request bodies, brace-balanced scope-aware SSE delta parsers (OpenAI + Anthropic)
  Tools/                     # KSP game data and domain knowledge
    ToolDefinitions.cs       # ToolSpec list (name, description, parameters JSON) → emits both OpenAI (GetToolsJsonArray) and Anthropic (GetToolsJsonArrayAnthropic) tool array formats; dispatch to GameDataTools
...
- **LLM streaming**: Uses `UnityWebRequest` with a custom `DownloadHandlerScript` subclass (`SseDownloadHandler`) to parse SSE chunks and accumulate tool call fragments. The handler takes a parser delegate so the same plumbing serves both providers — `JsonHelper.ParseStreamDelta` for OpenAI (walks brace/string structure to locate `choices[0].delta` then extracts `content`/`tool_calls` within that scope; avoids false matches in metadata-only chunks like OpenRouter usage/cost or Gemini finish chunks) and `JsonHelper.ParseAnthropicStreamEvent` for Anthropic (routes by top-level `type`: `content_block_start` with `tool_use`, `content_block_delta` with `text_delta`/`input_json_delta`; ignores `message_start`/`message_stop`/`ping`). Tool call slots are stored in a `SortedDictionary<int, _>` keyed by parser-supplied index — for OpenAI that's the tool-call array index, for Anthropic the content-block index (which may interleave with text blocks). No JSON library dependency — KSP's bundled Mono lacks `System.Xml.Linq`, which causes Newtonsoft.Json to fail type-load at addon-instantiation time. `StreamingUiLoop` coroutine drives a typewriter effect, throttled to ~10fps with change detection to avoid layout rebuild spam. During streaming, `_logBuilder.Length` snapshots enable efficient rollback without string copies. The final `ChatMessage` is created on completion.
- **Multi-provider request shaping**: `LlmClient.SendChatRequest` calls `ChatProviderDetector.Detect(BaseUrl)` to pick the protocol. OpenAI: POSTs to `{base}/chat/completions` with `Authorization: Bearer ...` and the standard tool-array body. Anthropic: POSTs to `{base}[/v1]/messages` (resolved by `BuildAnthropicUrl` — preserves an existing `/v1` suffix, otherwise appends `/v1/messages`) with `x-api-key` + `anthropic-version: 2023-06-01`, system prompt at top level (not as a message), `max_tokens: 4096`, tools using `input_schema`, assistant tool calls emitted as `content` blocks mixing `text` + `tool_use` (with the internal Arguments JSON string inlined raw as the `input` object), and consecutive tool results coalesced into a single user message of `tool_result` blocks.
