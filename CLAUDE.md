...
src/
  ...
  Api/                       # LLM communication
    ...
    JsonHelper.cs            # Newtonsoft.Json wrapper: escape, extract, build request body, scope-aware SSE delta parser
...
- **LLM streaming**: Uses `UnityWebRequest` with a custom `DownloadHandlerScript` subclass (`SseDownloadHandler`) to parse SSE chunks and accumulate tool call fragments. Each SSE payload is fed to `JsonHelper.ParseStreamDelta` which extracts `choices[0].delta.content` / `tool_calls` via Newtonsoft `JObject` — scope-aware so metadata-only chunks (OpenRouter usage/cost, Gemini finish chunks, empty-choices payloads) don't get mistaken for content or tool calls. `Newtonsoft.Json.dll` (~700KB) is bundled into `GameData/Kerpilot/Plugins/` at build time; risk: another mod loading a different Newtonsoft version into the shared AppDomain may cause version conflicts. `StreamingUiLoop` coroutine drives a typewriter effect, throttled to ~10fps with change detection to avoid layout rebuild spam. During streaming, `_logBuilder.Length` snapshots enable efficient rollback without string copies. The final `ChatMessage` is created on completion.
