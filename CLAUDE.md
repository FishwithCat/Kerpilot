...
src/
  ...
  Api/                       # LLM communication
    ...
    JsonHelper.cs            # Manual JSON utilities: escape, extract, build request body, brace-balanced scope-aware SSE delta parser
...
- **LLM streaming**: Uses `UnityWebRequest` with a custom `DownloadHandlerScript` subclass (`SseDownloadHandler`) to parse SSE chunks and accumulate tool call fragments. Each SSE payload is fed to `JsonHelper.ParseStreamDelta`, which walks brace/string structure to locate `choices[0].delta` and only then extracts `content` / `tool_calls` within that scope. This avoids false matches in metadata-only chunks (OpenRouter usage/cost, Gemini finish chunks, empty-choices payloads). No JSON library dependency — KSP's bundled Mono lacks `System.Xml.Linq`, which causes Newtonsoft.Json to fail type-load at addon-instantiation time. `StreamingUiLoop` coroutine drives a typewriter effect, throttled to ~10fps with change detection to avoid layout rebuild spam. During streaming, `_logBuilder.Length` snapshots enable efficient rollback without string copies. The final `ChatMessage` is created on completion.
